...
public abstract class IDataManager {
	 ...
	 */
	public abstract IMendixObject startSynchronizing(UserAction<?> action, Boolean applyEntityAcces) throws CoreException;

	protected void prepareForSynchronization(UserAction<?> action, Boolean applyEntityAcces) throws MendixReplicationException {
		if( !applyEntityAcces )
			this.settings.setContext( this.settings.getContext().createSudoClone() );
		else if( this.settings.importInNewContext() )
			this.settings.setContext( this.settings.getContext().clone() );
		...
		try {
			if( this.settings.getFinishingMicroflowName() != null ) {
				...
				if( this.settings.getFinishingMicroflowStatParamName() != null ) {
					replStatus = Core.instantiate(this.settings.getContext(), ReplicationStatus.getType());
					if( this.info != null ) {
					...
					if( this.tableMapping != null )
						replStatus.setValue(this.settings.getContext(), ReplicationStatus.MemberNames.ReplicationStatus_TableMapping.toString(), this.tableMapping.getId());
